...
def home(request):
    ...
    if request.method == 'POST':
        ...
        # authenticate if given user already exists
...
def register_user(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            # authenticate and login when user register themselves for the first time
...
def add_record(request):
    ...
    if request.user.is_authenticated:
        ...
        messages.success(request, "You need to be logged in to perform this action !")
        return redirect('home')
